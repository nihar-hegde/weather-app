...
Weather.prototype.loadAnimation = function () {
  ...
  // Function to hide loading and trigger animations when all expected AJAX requests are complete.
  ...
  function handleAjaxError() {
    ...
    handleAjaxComplete(); // Consider this request as complete even on error.
  }

  // Bind AJAX complete and error handlers to document.
...
Weather.prototype.getWeatherIcon = function (wId, sunrise, sunset) {
  //get weather icon passing returned ID of OpenWeatherMap API. Optional sunrise and sunset time, to determine if it's day or night type icon. Return an object with icon name and icon animation properties.
  if (wId) {
    var icon = {};
    icon.name = "na";
...
Weather.prototype.currentWeather = function () {
  //get current weather from OpenWeatherMap API, format, and display it.
  if (this.location) {
    function setMain(res) {
      if (res.main) {
        ...
        if (res.rain) {
          $("#precipitation").text(res.rain["1h"] + " mm");
        }
      }
    }
    ...
    $.getJSON("https://api.openweathermap.org/data/2.5/weather", {
      q: this.location,
      units: "metric",
      appid: "bc79ac499413d4de4469a32a2b7b263e"
    }, function (json) {
...
$(document).ready(function () {
  ...
  weather.setLocation();
  weather.setUnit();
  weather.currentWeather(); // Fetch current weather information when the page loads
});
